...
# Marco, 23.8.25. Ocio che per l'analisi MPI avevo usato sto file. 
...
indexes_MPI_tip = np.linspace(270, 290, 21, dtype=int)  # Indici per i punti di MPI associati alla 'Great Salinity Anomaly'

drho_djfm_tip = np.load(drho_path)[indexes_MPI_tip]  
nao_djfm_tip = np.load(nao_djfm_path)[indexes_MPI_tip]  

plt.scatter(drho_djfm_tip, nao_djfm_tip, alpha=0.7, marker='x',
            s=40, c=colors['MPI'])
            # Numerazione dei punti

# Marco, 8.10. Ho fatto modifichine sui numerini.
for idx, (x, y) in zip(indexes_MPI_tip, zip(drho_djfm_tip, nao_djfm_tip)):
    ...
    # else:
    plt.text(x, y, str(idx), fontsize=6, ha='right', va='bottom')
...
# Salvataggio
save_path = os.path.join(output_dir, 'scatterplot_Drho_NAO_8.10_allyears.png')
